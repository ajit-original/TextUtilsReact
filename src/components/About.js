...
export default function About() {
    ...
    const [btnText, btnSetText] = useState('Click to Enable Dark Mode')
    ...
    const handleViewMode = () => {
        if(myStyle.color == 'black'){
            ...
            btnSetText('Click to Enable Light Mode')
        }
        else{
            mySetStyle({
                color: 'black',
            ...
            btnSetText('Click to Enable Dark Mode')
        }
    }
  ...
  return (
    <div className='container my-3'>
        ...
        <div>
            <button class='btn btn-primary my-3 mx-2' onClick={handleViewMode}>{btnText}</button>
        </div>
    </div>
  )
}
